...
#窗口显示
def ShowWin(debug_img, red_mask):
...
def Trackbars(cfg):
    ...
    cv2.resizeWindow("Controls2", 500, 150)
    ...
    # 筛选
    cv2.createTrackbar("Area Min", "Controls", cfg.area_min, 5000, nothing)
    cv2.createTrackbar("CLAHE",    "Controls", int(cfg.use_clahe), 1, nothing)

    # 红色阈值 (只调 S/V，H 范围固定)
    cv2.createTrackbar("Red S",    "Controls2", int(cfg.red_lower1[1]), 255, nothing)
    cv2.createTrackbar("Red V",    "Controls2", int(cfg.red_lower1[2]), 255, nothing)
    cv2.createTrackbar("RedPxMin", "Controls2", cfg.red_pixel_min, 5000, nothing)
...
def update_trackbars(cfg):
    ...
    red_s = cv2.getTrackbarPos("Red S", "Controls2")
    red_v = cv2.getTrackbarPos("Red V", "Controls2")
    cfg.red_lower1 = np.array([0,   red_s, red_v])
    cfg.red_upper1 = np.array([10,  255,   255])
    cfg.red_lower2 = np.array([160, red_s, red_v])
    cfg.red_upper2 = np.array([180, 255,   255])
